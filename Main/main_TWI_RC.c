...
int main(int argc, char *argv[]){
  ...
  diff[0]=sqrt((probecoor[0]-traslcoor[0])*(probecoor[0]-traslcoor[0]));
  diff[1]=sqrt((probecoor[1]-traslcoor[1])*(probecoor[1]-traslcoor[1]));
  diff[2]=sqrt((probecoor[2]-traslcoor[2])*(probecoor[2]-traslcoor[2]));
  ...
  mylog("[MAIN]: Difference \n");
  mylog("[MAIN]: vecy=[%d,%d,%d]\n",diff[0],diff[1],diff[2]);
  if(lhs_measure)  
  ...
  gettimeofday(&t1, NULL);
  jac_rk(diff[0],diff[1],diff[2],dt);
  //jac_onepoint(diff[0],diff[1],diff[2]);
  gettimeofday(&t2, NULL);
  ...
  for(k=0;k<N_traj;k++){
	...
	for(np=0;np<NUM_PROB;np++){
	  //	  mylog("[MAIN]: <delta_V[%d]_test> %lf %e \n",np+1,0.,LHS_delta_V(np,traslpos,probepos,0,Phi,Phi));    
	  avg_vector[np]+=LHS_delta_V(np,traslpos,probepos,0,Phi,Phi);
	}
      }
	...
	for(np=0;np<NUM_PROB;np++)
	  for(ntop=0;ntop<NUM_OP;ntop++){
	    ...
	    avg_vector[NUM_PROB+np+NUM_PROB*ntop]+=RHS(traslpos,probepos,np,ntop,Phi,Phi);
	    }
      }
	...
	if((Flow_vec[count]-t-1)==0){
	  ...
	  if(lhs_measure){	  
	    for(np=0;np<NUM_PROB;np++){
	      //     mylog("[MAIN]: <delta_V[%d]_test> %lf %e \n",np+1,(t+1)*dt,LHS_delta_V(np,traslpos,probepos,t+1,Xi,Phi));
	      avg_vector[NUM_PROB*(NUM_OP+1)*(count+1)+np]+=LHS_delta_V(np,traslpos,probepos,count+1,Xi,Phi);
	    }
	  }
	  ...
	  if(rhs_measure){
		...
		avg_vector[NUM_PROB+np+NUM_PROB*ntop+NUM_PROB*(NUM_OP+1)*(count+1)]+=RHS(traslpos,probepos,np,ntop,Xi,Phi);
	  }	    
	}
	  count ++;
